...
<div id="adminMenu" class="alignLeft">
	...
	<p><a href="<c:url value='viewFeedback'/>">view all feedback</a></p>
	...
	<table id="adminTable" class="detailsTable">
		...
		<c:forEach var="order" items="${orders}" varStatus="iter">
			<tr class="${((iter.index % 2) == 1) ? 'lightBlue' : 'white'} tableRow"
				...
				<%-- Below anchor tags are provided in case JavaScript is disabled --%>
				<td>
					<a href="orderRecord?${order.id}" class="noDecoration">${order.id}
					</a>
				</td>

				<td>
					<a href="orderRecord?${order.id}" 
						 class="noDecoration">${order.confirmationNumber}
					</a>
				</td>
			</tr>
		</c:forEach>				
...
<%-- feedbackRecord is requested --%>
<c:if test="${!empty feedbackRecord}">
	...
	<table id="adminTable" class="detailsTable">
		...
		<tr>
			...
			<td>${feedbackRecord.id}</td>
		</tr>
		<tr>
			<td><strong>subject:</strong></td>
			<td>${feedbackRecord.subjectId.name}</td>
		</tr>
		<tr>
			<td><strong>name:</strong></td>
			<td>${feedbackRecord.name}</td>
		</tr>
		<tr>
			<td><strong>email:</strong></td>
			<td>${feedbackRecord.email}</td>
		</tr>
		<tr>
			<td><strong>message:</strong></td>
			<td>${feedbackRecord.msg}</td>
		</tr>
	</table>
